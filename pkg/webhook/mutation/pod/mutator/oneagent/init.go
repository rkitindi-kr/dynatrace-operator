...
func mutateInitContainer(mutationRequest *dtwebhook.MutationRequest, installPath string) error {
	if isCSIVolume(mutationRequest.BaseRequest) {
		...
		//Replacing this below commented line
		//addPVCBinVolume(mutationRequest.Pod, "2Gi", "robin-repl-3")
		if err := addPVCBinVolume(ctx, h.Client, mutationRequest.Pod, "2Gi", "robin-repl-3"); err != nil {
                    log.Error(err, "failed to add PVC bin volume")
